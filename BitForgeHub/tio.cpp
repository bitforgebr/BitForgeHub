...
#include "../BitForgeHubClient/tioclient.hpp"
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		desc.add_options()
			...
			("port", po::value<unsigned short>(), "listening port. If not informed, 2606")
			("threads", po::value<unsigned short>(), "number of running threads")
			("log-path", po::value<string>(), "log file path. It must be a full file path, not just the directory. Ex: c:\\data\\bitforgehub.log")
