...
//};
        ...
        [DllImport("bitforgehublientdll.dll", CallingConvention = CallingConvention.Cdecl)]
        ...
        public static void ThrowOnNativeApiError(int result)
        {
            if (result < 0)
            {
                throw new Exception("BitForgeHub error, number " + result.ToString());
            }
        }
    }
}
