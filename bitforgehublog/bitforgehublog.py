...
class BitForgeHubLoadToMemorySink(object):
  def __init__(self):
    self.containers = {}
...
class BitForgeHubReplaySink(object):
  def __init__(self, hub_address, batch_size = 1):
    self.hub = bitforgehubclient.connect(hub_address)
    self.containers = {}
...
class BitForgeHubLogResumeParser(object):
...
class BitForgeHubLogParser(object):
...
def main():
  ...
  print 'Loading file "%s" to BitForgeHub @ %s, %d msgs/sec, %s, %s' % \
  ...
  parser = BitForgeHubLogParser(
      BitForgeHubReplaySink(params.hub, batch_size = params.log_step),
